...
public class DAOMiCuenta implements IDAOMiCuenta {
    ...
    public ArrayList<DTOProductosHuesped> BuscarProductosHuesped(int idReservacion, int idHuesped) {
        ArrayList<DTOProductosHuesped> listaproductos = new ArrayList<>();
        try (Connection con = Conexion.getConnection()) {

            PreparedStatement pstmt = con.prepareStatement("select p.id,c.idHuesped,p.nombre,h.cantidad,p.precioUnitario from historialcuentapersonal h join producto p on h.idProducto=p.id join cuentapersonal c on h.idCuentaPersonal=c.id where c.idReservaHabitacion=? and c.idHuesped=?");
            pstmt.setInt(1, idReservacion);
            ...
            while (condicion == true) {
                if (respuesta.next()) {//si respuesta.next(revisa si hay un elemtento,salta al siguiente reistro) devuelve true=si encontro algo o false si no lo encontró
                    DTOProductosHuesped productos = new DTOProductosHuesped();

                    productos.setIdProducto(respuesta.getInt(1));
                    productos.setIdHuesped(respuesta.getInt(2));
                    productos.setNombre(respuesta.getString(3));
                    productos.setCantidad(respuesta.getString(4));
                    productos.setValortotal(respuesta.getString(5));
                    listaproductos.add(productos);
